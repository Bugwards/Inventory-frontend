...
export default function useItemMaster() {
  ...
  const refreshData = async () => {
    ...
    // Refresh the shared group list used by dropdown/filter
    await loadGroups();

    // If an item was created/updated while on Items tab,
    // refresh GroupsTable too because itemCount has changed.
    if (tab === "items") {
      await fetchData(
        "groups",
        0,
        false,
        states.groups.sortField || "name",
        states.groups.sortOrder || "ASC"
      );
    }
  };
