...
const menuItems = [
  ...
  { name: 'Item Master', icon: HiOutlineCube, href: '#' },
  { name: 'Stock & Bin Card', icon: FiArchive, href: '#' },
  { name: 'Goods Received', icon: FiFilePlus, href: '#' },
  { name: 'Requests', icon: FiFileText, href: '/stock-transfer/requests' },
  { name: 'Distribution', icon: FiSend, href: '#' },
  { name: 'Reports & Audit', icon: FiBarChart2, href: '/report' },
];
    ...
    <div className="w-64 bg-white h-screen border-r border-gray-200 flex flex-col pt-4 pb-6 px-4">
      ...
      <nav className="flex-1 space-y-1">
        {menuItems.map((item, index) => {
          ...
          const isActive = pathname === item.href || (item.name === 'Requests' && pathname?.includes('/stock-transfer'));
